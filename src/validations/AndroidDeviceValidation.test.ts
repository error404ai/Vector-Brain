...
export const DispatchAndroidPromptValidation = z.object({
  ...
  // Matches the 1-200 range the dashboard offers. The old cap of 50 silently
  // rejected any longer run with a generic validation error.
  max_steps: z.number().min(1, 'At least 1 step is required').max(200, 'Step limit cannot exceed 200').default(15),
});
